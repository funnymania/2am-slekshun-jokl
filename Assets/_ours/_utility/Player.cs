...
public class Player : MonoBehaviour {
    ...
    void Update () {
		if (!EventHandler.eventFlag) {
			...
			/*if(Input.GetAxis("Start")>0 && blurb){
				...
				yada.enabled=true;}*/
			camRotorPast = camRotor;
            camRotor = camRotation.eulerAngles.y;
			...
			if (Input.GetAxis("Eyes") == 0 && Lshift) {
                LshiftDown = false;
            }
					#region Rotation
			// + -
				...
				if (horiz != 0) {
					...
					if (charState != CharacterState.Strafe) {
						...
						if (charState == CharacterState.Skip) {
                            ...
                            turnTranny.eulerAngles = new Vector3(turnTranny.localEulerAngles.x, (float)(rotor), turnTranny.localEulerAngles.z);
                        }
                    } else {	
            ...
            {		    
				...
				if (enemyInfo.inHitstun) {
					...
					altDown = true;
					altDepressed = false;
                    charState = CharacterState.Sitting;
                } else if (weaponCoolDown > 7) {
					if (Input.GetAxis("Select") < 0) {
						if (daggerOut) {
							charState = CharacterState.Conjure;
                            daggerOut = false;
                            magicMax = 100;
							entrance = true;
                        } else {	
				...
				if(horiz != 0 || vertic != 0) {
                    ...
                    isIdle = true;
                    timer = 0;
                    charStateLast = CharacterState.Turning;
					chance = Random.Range(2, 6) * 2.667F;
                    idleState = 0;
                    transNow = false;
					charSpeed = 0;
			...
			{   
                ...
                if (enemyInfo.inHitstun) {
                    ...
                    isIdle = true;
                    timer = 0;
                    charStateLast = CharacterState.Trotting;
					chance = Random.Range(2, 6) * 2.667F;
                    idleState = 0;
                    transNow = false;
					charSpeed = 0;
                    ...
                    dashDanceTimer = 0;
					endurVelocity = 40;
                    clearMe.clearRemains();
					if (lastHoriz > 0) 
                        meshan.CrossFade("idleRight", 0F);
					else
                        meshan.CrossFade("idleLeft", 0F);
			...
			else if (charState == CharacterState.Sprint)
			{
				if (enemyInfo.inHitstun) {
					...
					charState = CharacterState.HeavySkid;
                    timer = 0;
                }
			}
			#endregion
			...
			else if (charState == CharacterState.Attack)
			{
				...
				if (meshan.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.126 &&
				    meshan.GetCurrentAnimatorStateInfo(0).normalizedTime < 0.166) {
					for (i = 0; i < HitBoxes.Count; i++) {
						HitBoxes[i].enabled = true;
                    }
					attackDown2 = true;
					enemyInfo.theirPain = 8;
                    entrance = false;
					if (Input.GetAxis("Attack") > 0) 
                        attackDown = true;
                } else if (meshan.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.35 &&
						   meshan.GetCurrentAnimatorStateInfo(0).normalizedTime < 0.526) {
					for (i = 0; i < HitBoxes.Count; i++) {
						HitBoxes[i].enabled = true;
                    }
					attackDown2 = true;
                    enemyInfo.theirPain = 10;
					if (Input.GetAxis("Attack") > 0) 
                        attackDown = true;
                    ...
                    attackDown2 = false;
					charSpeed = 0;
                    chance = Random.Range(6, 17) * 2.667F;
                    idleState = 0;
					endurVelocity = 20;
                    meshan.CrossFade("Take 001", 0.1F);
			...
			else if (charState == CharacterState.Walking)
			{
                ...
                if (enemyInfo.inHitstun) {
                    ...
                    isWalk = false;
                    charStateLast = CharacterState.Walking;
					isStrafe = true;
                } else if ((horiz > 0.6F || vertic > 0.6F || horiz < -0.6F || vertic < -0.6F) && !isWalk) {
                    footDown = true;
                    charState = CharacterState.Trotting;
                    breathAmt = 1.5F;
                } else if (horiz > 0.3F || vertic > 0.3F || horiz < -0.3F || vertic < -0.3F) {
					charSpeed = 40;
                } else if (horiz != 0 || vertic != 0) {
					charSpeed = 32;
                } else if (horiz == 0 && vertic == 0) {
			...
			else if (charState == CharacterState.Strafe)
			{
				if (Input.GetAxis("Eyes") != 0) {
					if ((Mathf.Abs(horiz) >= 0.9F || Mathf.Abs(vertic) >= 0.9F) && !hardStrafe) {
                        ...
                        endurance -= 40;
                        strafing = true;
						meshan.CrossFade("LimenousForwardEnter", 0.1F);
                        strafeSwitch = true;
					...
					if (strafeTimer2 > 0.2F) {
                        ...
                        isIdle = true;
                        timer = 0;
                        charStateLast = CharacterState.Skip;
						chance = Random.Range(2, 6) * 2.667F;
                        idleState = 0;
                        transNow = false;
						charSpeed = 0;
                        ...
                        strafeJumpTimer = 0;
                        charSpeed = 880;
						CloudMain.transform.position = tr.position;
                        CloudMain.transform.localScale = new Vector3(0.2F, 0.2F, 0.2F);
			...
			else if (charState == CharacterState.Jump)
            {
				if (hurtSquib) {
                    ...
                    if (Input.GetAxis("LBumper") > 0 && ledgeNearby){
                        ...
                        isIdle = true;
                        timer = 0;
                        charStateLast = CharacterState.Jump;
                        chance = Random.Range(2, 6) * 2.667F;
                        idleState = 0;
                        transNow = false;
                        endurVelocity = 40;
				...
				if (timer >= transAnimTime) {
					if (charStateLast == CharacterState.Ledge) {
						if (ledgeType == "floor") {	
                            ...
                            isIdle = true;
                            timer = 0;
							chance = Random.Range(6, 17) * an["Idle"].length;
                            idleState = 0;
                            an["OneArmBehindBack"].speed = -1;
			...
			else if (charState == CharacterState.Sitting)
			{
				if (meshan.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1) {
                    ...
                    isIdle = true;
                    timer = 0;
                    charStateLast = CharacterState.Sitting;
					chance = Random.Range(2, 6) * 2.667F;
                    idleState = 0;
                    transNow = false;
                    meshan.speed = 1;
	...
	void OnGUI () {
		if(enemyInfo.health > 0) {
			...
			GUI.DrawTexture(new Rect(cameraBlah.toResize[1].rect.x * (float)Screen.width, 
                                     cameraBlah.toResize[0].rect.y * (float)Screen.height,
			                         cameraBlah.toResize[0].rect.width * (float)Screen.width,
                                     cameraBlah.toResize[0].rect.height * (float)Screen.height),
			                         gameOver,
