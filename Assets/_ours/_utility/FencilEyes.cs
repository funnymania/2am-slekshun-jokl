...
public class FencilEyes : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetAxis("Squint") != 0) {
			...
			if (Physics.Raycast(tr.position, tr.forward, out presentEye, 24)) {
                Debug.DrawRay(tr.position, tr.forward * 24);
				if (presentEye.transform.name == "lineSighting") {
					work = (tr.position - presentEye.transform.position).sqrMagnitude;
					if (work == 0) {
						work = 1;
                        col = new Color(0, .6F, .765F, 1);
                    } else {	
                        work = 1 - work / 576;
                        col = new Color(0, .6F * work, .765F * work, 1);
                    }
                } else {	
                    col = new Color(0,0,0,1);
                }
			...
			if (Physics.Raycast(tr.position, tr.forward, out presentEye, 12)) {
                Debug.DrawRay(tr.position, tr.forward * 8);
				if (presentEye.transform.name == "lineSighting") {
					work = (tr.position - presentEye.transform.position).sqrMagnitude;
					if (work == 0) {
						work = 1;
