...
public class StartScreen : MonoBehaviour {
	...
	void Start(){
		...
		disable.enabled=false;booya.enabled=true;tr=transform;
		...
		clone.transform.position=new Vector3(PinealSpace.position.x,PinealSpace.position.y-.018F,PinealSpace.position.z);
		clone.transform.rotation=tr.rotation;
		sowa=clone.GetComponentInChildren<StartScreen>(); // Clone's stuff.
		booya=clone.GetComponentInChildren<WhileStart>();
		sowa.enabled=false;booya.enabled=false;
		fartClone=GameObject.Find("Player(Clone)/Luyaka/polySurface424");
		disable=clone.GetComponentInChildren<Player>();
		disable.tinyDude=.008F; CameraHell.isTiny=true;
		color=fartClone.GetComponent<Renderer>().material.color;color.a=0;		// Not Making transparent
		fartClone.GetComponent<Renderer>().material.color=color;
		squash=FartInMyFace.GetComponent<Renderer>().material.color;squash.a=1; // Luya skin
	...
	void Update(){
		if(timer>=3){
			//gameObject.SetActive(false); //reverse normals on Luyaka (when camera penetrates)
			color.a+=Time.deltaTime/1.5F;
			fartClone.GetComponent<Renderer>().material.color=color;
			
			if(timer>4.5F){
				Player.startFlag=false;disable.enabled=true;disable.blurb=false;
				CameraHell.tr.position=new Vector3(clone.transform.position.x+(6.4F*.008F),clone.transform.position.y,
		...
		{	CameraHell.tr.position=Vector3.Lerp(cameraStart.position,PinealSpace.position,timer/3.6F); // derp
			squash.a-=Time.deltaTime/3;  // Luya Body becoming transparent
			FartInMyFace.GetComponent<Renderer>().material.color=squash;}
