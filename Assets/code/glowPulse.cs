...
public class glowPulse : MonoBehaviour {
	...
	void Update () { /* Blur spread Down to .5, blur spread to .2 then GlowIntens to .337 */
		if(goingDown){
			if(downAgain){
				time+=Time.deltaTime;
				tmp.BlurSpread=blurBegin-(blurGap*time/timeDown*2);
				if(time>=timeDown/2){ 
					downAgain=false;blurMid=tmp.BlurSpread;
					blurGap2=blurMid-blurEnd;time=0;}}
			else
			{	time+=Time.deltaTime;
				tmp.BlurSpread=blurMid-(blurGap2*time/timeDown*2);
				tmp.GlowIntensity=intensBegin-(intensGap*time/timeDown*2);
				if(time>=timeDown/2){
					goingDown=false;downAgain=true;
					blurGap2=blurMiddle-tmp.BlurSpread;time=0;
					intensGap=intensBegin-tmp.GlowIntensity;
					blurMid=tmp.BlurSpread;intensEnd=tmp.GlowIntensity;}}}
		...
		{	if(upAgain){
				...
				if(time>=timeUp/2){ 
					upAgain=false;blurMid=tmp.BlurSpread;
					blurGap=blurBegin-blurMid;time=0;
					intensBegin=tmp.GlowIntensity;}}
			else
			{	time+=Time.deltaTime;
				tmp.BlurSpread=blurMid+(blurGap*time/timeUp*2);
				if(time>=timeUp/2){
					goingDown=true;upAgain=true;time=0;
					blurGap=tmp.BlurSpread-blurMiddle;}}}
