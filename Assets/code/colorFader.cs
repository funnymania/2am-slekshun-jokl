...
public class colorFader : MonoBehaviour {
	...
	void Start(){
		...
		else rand=new Color(Random.Range(0F,0.2F),Random.Range(0F,.87F),Random.Range(0.9F,1.0F));
	...
	void Update () {
		if(t>=speed){
			...
			else rand=new Color(Random.Range(0.0F,0.3F),Random.Range(0.67F,1F),Random.Range(0.7F,.9F));
			t-=speed;
			go=Color.Lerp(first,rand,t/speed);
			if(me!=null)
				me.color=go;
			else if(moi!=null) moi.color=go;
			else if(cammy!=null) cammy.backgroundColor=go;
			t+=Time.deltaTime;
			}
		else 
		{	go=Color.Lerp(first,rand,t/speed);
