			...
			for(i=0;i<stumps4fade.Count;i++){
				for(j=0;j<stumps4fade[i].Count;j++)
					if(blah.parent==stumps4fade[i][j].parent){
						stumps4fade[i].Add(blah);break;}
				if(j!=stumps4fade[i].Count){
					break;}
			}
				
				//fadeMe.Add(please);
				//tLerpers.Add((Player.myTr.position-blah.position).sqrMagnitude);}
...
/// workIt global var set by Player
/// 
/// 
///
///
		...
		if(workIt && localWork){
			for(i=0;i<stumps4fade.Count;i++){
				if(stumps4fade[i].Count==1){
					...
					yada.a=.28F-(.28F*(stumps4fade[i][0].position-Player.tr.position).sqrMagnitude/600); /* maybe use camera position */
				...
				{	dist=(stumps4fade[i][0].position-Player.tr.position).sqrMagnitude;
					for(j=1;j<stumps4fade[i].Count;j++){
						...
						/* find shortest distance and calc from that*/
				}
			}
		}
		else if(!workIt) localWork=false;
	}
...
*/
	void OnTriggerEnter(Collider col){
		if(col.tag=="Player"){
			if(!localWork)	/// This means: if entering line, start fade behaviors. Otherwise, stop.
				localWork=true;
			else localWork=false;}
	}
}
