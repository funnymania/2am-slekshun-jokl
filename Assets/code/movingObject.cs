	...
	void OnAnimatorMove(){
		if(Player.onLine && onBlock){
			Player.straightDown=Physics.Raycast(
						blah.transform.position,new Vector3(0,-1,0),out Player.downwards,.98F);
...
/* 
if(Player.downwards.transform.tag!="floor")					
					if(mover!=Player.downwards.transform) 
						mover=Player.downwards.transform;
				else if(Player.downwards.transform.tag=="floor")
					Player.onLine=false;}
	
	
	if onLine the selection of mover belongs to movingObject
	
	if(Player.downwards.transform!=null && Player.downwards.transform.tag=="floor")
			{
				Player.onLine=false;}
			else
			{	blah.Move(mover.position-past);
